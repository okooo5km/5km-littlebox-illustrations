...
Do not illustrate every paragraph. Prefer high-leverage moments: decision points, before/after states, intake-to-output systems, evidence accumulation, scattered-to-structured transformations, hidden bottlenecks, recurring mistakes, and final handoff. Simplify only after the idea is fully expressed: clarity and semantic completeness outrank a fixed object count or a large empty area.
...
- visual metaphor
- background: pure white `#FFFFFF` by default, or the user's exact requested color
- suggested objects
- composition structure and visual counterweight
...
When the user does not mention a background, assign pure white `#FFFFFF` to every image, including multi-image sets. If the user asks for alternatives, offer very pale cream `#FFFCF5`, pale sky blue `#E3F2FD`, and pale lavender `#E6E6FA`. If the user specifies any other background color, use that exact color instead. Do not randomize or balance background colors unless the user explicitly asks for variation.
...
- use one core idea only, but include every object needed to make that idea understandable
...
- choose or preserve the assigned background from the planning step; default to pure white `#FFFFFF`, and let an explicit user-specified color override every preset
- usually use 3-7 supporting elements; use fewer or more when the concept genuinely needs it, provided the hierarchy stays legible
- use short visible labels in the user's language
- distribute visual weight across the canvas with staggered heights, depth, scale, and counterweights; avoid placing all elements on one horizontal track
- choose a non-linear composition by default: orbit, diagonal tension, triangular balance, staggered clusters, field composition, or asymmetric counterweight
- keep breathing room around meaningful groups, but never delete necessary content just to manufacture emptiness
...
- color is overused
- the image has no clear hierarchy or the supporting details compete with the core idea
