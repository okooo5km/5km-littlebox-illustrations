...
>
> 16:9 横版为主 | 小盒 IP | 合盖纸盒小人 | 中粗手绘马克笔 | 均衡非线性构图 | 默认纯白背景 | Codex Skill
...
- 默认纯白 `#FFFFFF` 平面背景，在浅色 UI 和文章页面中保持无边界沉浸感
- 用户可以自由指定背景色；内置候选为极浅奶油白 `#FFFCF5`、浅天蓝 `#E3F2FD`、浅薰衣草紫 `#E6E6FA`
- 黑色中粗马克笔线条，比旧版略细一档；主次线条有层级，保留干刷质感和粗糙边缘
- 小盒永远合盖，不开盖、不半开、不剖面
- 小盒白色盒身，黑点眼睛，小短腿，两侧小细枝胳膊
- 顶部中央只有一条琥珀色锯齿胶带
- 珊瑚红只用于警告、盖章、错误或强调
- 中文标签短、少、手写感强
- 一张图只表达一个核心观点，但以把内容讲清楚为先，不为追求空而删掉必要信息
- 默认使用环绕、对角张力、三角平衡、错落组团或非对称配重，避免所有元素排成横向一条线
- 留白用于分隔语义组，画面重心在横纵两个方向上都要协调
- 小盒必须参与核心动作，不能只是站在旁边
...
![技术债利息](examples/images/01-tech-debt-interest.jpeg)
...
![通胀压力篮](examples/images/02-economics-inflation-baskets.jpeg)
...
![拖延不是懒](examples/images/03-psychology-procrastination-clarity.jpeg)
...
![合同小字](examples/images/04-law-contract-small-print.jpeg)
...
![生态边界](examples/images/05-ecology-system-boundary.jpeg)
...
![家庭开销漏点](examples/images/06-household-spending-leaks.jpeg)
...
![症状搜索别下结论](examples/images/07-health-search-maybe.jpeg)
...
![育儿建议筛选](examples/images/08-parenting-advice-filter.jpeg)
...
![通勤吞掉生活](examples/images/09-commute-eats-life.jpeg)

示例图均由 PNG 源图经 Zipic 转换并压缩为 JPEG，在保持手写标签和马克笔边缘清晰的同时降低仓库与页面加载体积。仓库只保留用于展示的 JPEG 成品。
...
要求：16:9 横版、默认纯白背景、中粗黑色马克笔、主次线条有层级、少量琥珀色和珊瑚红中文手写标注，构图不要排成横向一条线。需要其他背景色时按用户指定值生成。
...
4. 默认使用纯白背景；用户指定颜色时优先使用指定值，需要候选时提供极浅奶油白、浅天蓝和浅薰衣草紫
5. 重新发明一个低科技、怪诞但成立的物理隐喻
6. 让小盒在合盖状态下承担核心动作
7. 每张图单独调用图像模型生成
8. 按 quality gate 检查：合盖、胶带、胳膊、线条层级、内容完整、构图配重、中文标注、非 PPT 感
9. 保存最终 PNG，并报告路径
...
├── examples/
│   ├── images/
│   │   ├── 01-tech-debt-interest.jpeg
│   │   ├── 02-economics-inflation-baskets.jpeg
│   │   ├── 03-psychology-procrastination-clarity.jpeg
│   │   ├── 04-law-contract-small-print.jpeg
│   │   ├── 05-ecology-system-boundary.jpeg
│   │   ├── 06-household-spending-leaks.jpeg
│   │   ├── 07-health-search-maybe.jpeg
│   │   ├── 08-parenting-advice-filter.jpeg
│   │   └── 09-commute-eats-life.jpeg
│   ├── prompts.md
│   └── theme-prompts.md
└── 5km-littlebox-illustrations/
    ├── SKILL.md
